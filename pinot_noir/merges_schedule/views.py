...
def index(request):
    ...
    assignees = sorted(
        {(m.assignee_user.username if m.assignee_user else "UNASSIGNED") for m in merges}
    )
    statuses = [
        s for s, _ in sorted(Merge.STATUS_CHOICES, key=lambda item: STATUS_ORDER.get(item[0], -1))
    ]
